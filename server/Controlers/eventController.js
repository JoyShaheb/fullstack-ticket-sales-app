  ...
  try {
    ...
    jwt.verify(token, process.env.JWT_SECRET_KEY, async (err, decodedToken) => {
      ...
      if (decodedToken.role !== "admin") {
        return res.status(400).json({
          message:
            "You are not authorized to update events. Please log in as an admin.",
        });
      }

      const { title, description, date, location, image, price, type } =
        req.body;
  ...
  try {
    ...
    jwt.verify(token, process.env.JWT_SECRET_KEY, async (err, decodedToken) => {
      ...
      if (decodedToken.role !== "admin") {
        return res.status(400).json({
          message:
            "You are not authorized to delete events. Please log in as an admin.",
        });
      }
